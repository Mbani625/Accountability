...
function openEditTaskModal(taskItem) {
    ...
    document.getElementById('edit-task-duration').value = taskItem.dataset.duration;
    document.getElementById('edit-reminder-interval').value = taskItem.dataset.reminder;
...
saveTaskButton.addEventListener('click', () => {
    ...
    currentTaskElement.querySelector('.gear-icon').addEventListener('click', (e) => {
        e.stopPropagation();
...
function updateCounts() {
    ...
    document.getElementById('profile-delayed-completed-count').textContent = delayedCompletedTasks;
    ...
    const successRate = totalTasks > 0 ? Math.floor((completedCount / totalTasks) * 100) : 0;
    const delayedRatio = totalTasks > 0 ? Math.floor((delayedCompletedTasks / totalTasks) * 100) : 0;

    document.getElementById('profile-success-rate').textContent = `${successRate}%`;
    document.getElementById('profile-delayed-ratio').textContent = `${delayedRatio}%`;
